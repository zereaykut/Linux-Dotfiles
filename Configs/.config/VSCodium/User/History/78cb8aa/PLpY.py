...
def main():
    ...
    # 4B) NEW: Convergence_850 (from u/v at 850 hPa)
    Convergence_850 = np.full_like(CAPE_Proxy, np.nan, dtype=np.float32)
    if has_u and has_v and (u850_all is not None) and (v850_all is not None):
        print("Computing 850 hPa convergence (time loop)...")
        for t in range(ds.sizes["time"]):
            if (t + 1) % 10 == 0 or (t + 1) == ds.sizes["time"]:
                print(f"  divergence850 {t+1}/{ds.sizes['time']}", end="\r")

            u850 = u850_all[t] * units("m/s")
            v850 = v850_all[t] * units("m/s")
            div850 = mpcalc.divergence(u850, v850, dx=dx, dy=dy)  # 1/s
            Convergence_850[t] = (-div850).to("1/s").m.astype(np.float32)
        print()
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            "notes": (
                ...
                "Convergence_10m computed via -div(u10,v10) if u10/v10 exist. "
                "Convergence_850 computed via -div(u850,v850). "
                "LI/SI left as NaN unless you implement hotspot-only."
            )
        },
    )
