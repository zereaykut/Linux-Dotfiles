...
def main():
    ...
    # Full Td_mean across 1000/950/850/700/500
    Td_mean = ((Td_1000 + Td_950 + Td_850 + Td_700 + Td_500) / 5.0).astype(np.float32)
    ...
    DD_500  = (t500  - Td_500 ).astype(np.float32)

    # Low-layer mean DD (1000/950/850)
    DD_LowMean_1000_850 = ((DD_1000 + DD_950 + DD_850) / 3.0).astype(np.float32)

    # Full DD_mean across 1000/950/850/700/500
    DD_mean = ((DD_1000 + DD_950 + DD_850 + DD_700 + DD_500) / 5.0).astype(np.float32)

    # CAPE proxy uses dd_mean (already)
    ...
    # ------------------------------------------------------------
    # Convergence + Relative Vorticity calculations (time loop)
    # ------------------------------------------------------------
    lon2d, lat2d = np.meshgrid(ds.longitude.values, ds.latitude.values)
    dx, dy = mpcalc.lat_lon_grid_deltas(lon2d, lat2d)

    # Convergence_10m and Vorticity_10m
    Convergence_10m = np.full_like(CAPE_Proxy, np.nan, dtype=np.float32)
    Vorticity_10m = np.full_like(CAPE_Proxy, np.nan, dtype=np.float32)

    if has_u10 and has_v10:
        print("Computing 10m convergence & relative vorticity (time loop)...")
        ...
        for t in range(ds.sizes["time"]):
            if (t + 1) % 10 == 0 or (t + 1) == ds.sizes["time"]:
                print(f"  10m div/vort {t+1}/{ds.sizes['time']}", end="\r")

            u10 = u10_all[t] * units("m/s")
            v10 = v10_all[t] * units("m/s")

            div = mpcalc.divergence(u10, v10, dx=dx, dy=dy)  # 1/s
            vort = mpcalc.vorticity(u10, v10, dx=dx, dy=dy)   # 1/s (relative vorticity)

            Convergence_10m[t] = (-div).to("1/s").m.astype(np.float32)
            Vorticity_10m[t] = vort.to("1/s").m.astype(np.float32)
        print()
    ...
    def compute_convergence_level(level_hpa: int):
        ...
        if not _has_level(ds, level_hpa):
            print(f"WARNING: isobaric level {level_hpa} hPa not found. Convergence_{level_hpa} will be NaN.")
            return out
        ...
        for t in range(ds.sizes["time"]):
            if (t + 1) % 10 == 0 or (t + 1) == ds.sizes["time"]:
                print(f"  div{level_hpa} {t+1}/{ds.sizes['time']}", end="\r")
    ...
    # Helper to compute relative vorticity at a pressure level
    def compute_vorticity_level(level_hpa: int):
        out = np.full_like(CAPE_Proxy, np.nan, dtype=np.float32)
        if not (has_u and has_v):
            return out

        if not _has_level(ds, level_hpa):
            print(f"WARNING: isobaric level {level_hpa} hPa not found. Vorticity_{level_hpa} will be NaN.")
            return out

        u_all = ds["u"].sel(isobaricInhPa=level_hpa).values.astype(np.float32)
        v_all = ds["v"].sel(isobaricInhPa=level_hpa).values.astype(np.float32)

        print(f"Computing {level_hpa} hPa relative vorticity (time loop)...")
        for t in range(ds.sizes["time"]):
            if (t + 1) % 10 == 0 or (t + 1) == ds.sizes["time"]:
                print(f"  vort{level_hpa} {t+1}/{ds.sizes['time']}", end="\r")

            u = u_all[t] * units("m/s")
            v = v_all[t] * units("m/s")
            vort = mpcalc.vorticity(u, v, dx=dx, dy=dy)  # 1/s
            out[t] = vort.to("1/s").m.astype(np.float32)
        print()
        return out
    ...
    Convergence_850  = compute_convergence_level(850)

    # NEW: mean convergence across 1000/950/850 (ignore NaNs)
    Convergence_mean = np.nanmean(
        np.stack([Convergence_1000, Convergence_950, Convergence_850], axis=0),
        axis=0
    ).astype(np.float32)

    # NEW: Relative vorticity at pressure levels + mean
    Vorticity_1000 = compute_vorticity_level(1000)
    Vorticity_950  = compute_vorticity_level(950)
    Vorticity_850  = compute_vorticity_level(850)

    Vorticity_mean = np.nanmean(
        np.stack([Vorticity_1000, Vorticity_950, Vorticity_850], axis=0),
        axis=0
    ).astype(np.float32)
    ...
    ds_out = xr.Dataset(
        data_vars={
            ...
            "DD_500":  (("time", "latitude", "longitude"), DD_500),

            "DD_LowMean_1000_850": (("time", "latitude", "longitude"), DD_LowMean_1000_850),
            "DD_mean": (("time", "latitude", "longitude"), DD_mean),
            ...
            "Convergence_mean": (("time", "latitude", "longitude"), Convergence_mean),

            "Convergence_10m": (("time", "latitude", "longitude"), Convergence_10m),

            # NEW: Relative vorticity outputs
            "Vorticity_1000": (("time", "latitude", "longitude"), Vorticity_1000),
            "Vorticity_950":  (("time", "latitude", "longitude"), Vorticity_950),
            "Vorticity_850":  (("time", "latitude", "longitude"), Vorticity_850),
            "Vorticity_mean": (("time", "latitude", "longitude"), Vorticity_mean),
            "Vorticity_10m":  (("time", "latitude", "longitude"), Vorticity_10m),
        ...
        attrs={
            "notes": (
                "FAST build: Td computed only at 1000/950/850/700/500 hPa; indices vectorized. "
                "DD_mean is mean dewpoint depression across 1000/950/850/700/500. "
                "Convergence_mean is mean of Convergence_1000/950/850 (NaN-safe). "
                "Vorticity_mean is mean of Vorticity_1000/950/850 (NaN-safe). "
                "Relative vorticity computed via metpy.calc.vorticity using lat/lon grid deltas."
            )
        },
    )
