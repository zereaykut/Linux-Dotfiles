...
set env_file "$HOME/.config/environment.d/custom.conf"

if test -f "$env_file"
    # Read file, ignore comments, convert 'KEY=VAL' to 'set -gx KEY VAL', and execute
    sed -E 's/^([^#[:space:]]+)=(.*)$/set -gx \1 \2/' "$env_file" | source
end
