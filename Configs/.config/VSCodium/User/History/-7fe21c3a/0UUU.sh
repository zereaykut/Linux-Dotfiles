...
# elif [[ "$SESSION" == *"wayfire"* ]]; then
#     echo "Detected Wayfire session"
#     cp -f $theme_path/wayfire.ini $HOME/.config/wayfire/theme.ini

#     # Reload Wayfire config by sending SIGHUP
#     if pgrep -x wayfire >/dev/null; then
#         pkill -HUP wayfire
#         echo "Wayfire config reloaded"
#     else
#         echo "Wayfire process not found"
#     fi
else
