...
class NiriClient:
    ...
    def preprocess_data(self):
        ...
        for window in self.windows:
            app_id = window.get("app-id", "")
            title = window.get("title", "")
            ws = window.get("workspace", "")
            handle = window.get("handle", "")
            
            if app_id:
                # app-id uzun olabiliyor (Firefox: "org.mozilla.firefox")
                short_class = app_id.split(".")[-1] if app_id.startswith("org.") else app_id

                rofi_list.append(
                    {"ws": ws, "class": short_class, "title": title, "handle": handle}
                )
    ...
    def create_rofi_output(self):
        ...
        for window in self.rofi_list:
            ...
            entry = f"ws {ws:^2} | {window_class:<20} <> {title:<45}   || {handle:^10}"
    ...
    def focus_window(self, handle):
        sp.run(["niri", "msg", "focus-window", handle])

    def run(self):
        rofi_output = self.create_rofi_output()
        selected_handle = self.get_rofi_selection(rofi_output)

        if selected_handle:
            self.focus_window(selected_handle)
