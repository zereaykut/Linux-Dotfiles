...
class NiriClient:
    ...
    def preprocess_data(self):
        ...
        for window in self.windows:
            ...
            if app_id:
                ...
                rofi_list.append(
                    {"ws": ws, "class": short_class, "title": title, "app_id": app_id}
                )
    ...
    def create_rofi_output(self):
        ...
        for window in self.rofi_list:
            ...
            entry = f"ws {ws:^2} | {window_class:<20} <> {title:<45}   || {app_id:^10}"
    ...
    def focus_window(self, app_id):
        sp.run(["niri", "msg", "action", "focus-window", app_id])

    def run(self):
        rofi_output = self.create_rofi_output()
        selected_app_id = self.get_rofi_selection(rofi_output)

        if selected_app_id:
            self.focus_window(selected_app_id)
