...
if [ "${file_extension,,}" == "png" ]; then
    ...
    cp "$wall_path" "$HYPRLOCK_CACHE/lock_screen.png"
    echo "PNG image copied to $HYPRLOCK_CACHE/lock_screen.png"
else
    # Otherwise, convert the image to PNG using ImageMagick and copy it to the folder
    $command "$wall_path[0]" "$HYPRLOCK_CACHE/lock_screen.png"
    echo "Image converted to PNG and copied to $HYPRLOCK_CACHE"
fi
