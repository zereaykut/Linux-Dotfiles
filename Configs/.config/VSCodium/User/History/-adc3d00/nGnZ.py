...
def main():
    ...
    # -------------------------------
    # 🔧 ADD THIS (uniform Zarr-safe chunks)
    # -------------------------------
    ...
    ds_tpl = open_zarr_store(CACHE / "t_pl_low.zarr", chunks=target_chunks)
    ds_upl = open_zarr_store(CACHE / "u_pl_low.zarr", chunks=target_chunks)
    ds_vpl = open_zarr_store(CACHE / "v_pl_low.zarr", chunks=target_chunks)
    ...
    for t0, t1 in month_range("2024-06-01", "2024-09-01"):
        ...
        t2m_s, d2m_s, u10_s, v10_s, t950_s, t850_s, u850_s, v850_s = xr.align(
            t2m_s, d2m_s, u10_s, v10_s,
            t950_s, t850_s, u850_s, v850_s,
            join="inner"
        )

        # Rule 4: compute CAI lazily with dask
        ...
        # -------------------------------
        # 🔧 MINIMAL FIX (THIS IS THE KEY LINE)
        # -------------------------------
        cai = cai.chunk(target_chunks)

        # Write CAI safely to Zarr
        cai_path = OUT / f"cai_{t0}_to_{t1}.zarr"
        cai.to_dataset(name="CAI").to_zarr(cai_path, mode="w")
        print(f"Saved CAI Zarr: {cai_path}")

        # Rule 5: compute ONLY subset period for tracking
