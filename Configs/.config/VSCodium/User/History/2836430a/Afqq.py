...
def model_data(df_target:pd.DataFrame, df_mto:pd.DataFrame, index: str, start: str, end: str)->pd.DataFrame:
    df = pd.merge(df_target[(df_target[index]>=start) & (df_target[index]<=end)], df_mto[(df_mto[index]>=start) & (df_mto[index]<=end)], on=index, how="outer")
    return df
