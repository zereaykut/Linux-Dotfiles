...
if [ -z "$APP" ]; then
    notify-send "System Error" "No application name provided to the toggle script." -u critical
    exit 1
fi

# 3. Check if the application exists (is installed/executable)
if ! command -v "$APP" >/dev/null 2>&1; then
    notify-send "System Error" "Application '$APP' not found or not executable." -u critical
    exit 1
fi
