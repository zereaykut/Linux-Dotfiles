...
def model_prediction(df:pd.DataFrame, target: str, model_name: str)->None:
    plt.figure(figsize=(14, 6))

    plt.subplot(1, 2, 1)
    plt.scatter(df[target], df["Prediction"], alpha=0.5, color="blue", label="Predictions")
    # plt.plot([df.min(), df.max()], [df.min(), df.max()], "r--", lw=2, label="Perfect Fit")
    plt.title(f"{model_name}\nRMSE: {rmse_lr:.3f}, R2: {r2_lr:.3f}")
