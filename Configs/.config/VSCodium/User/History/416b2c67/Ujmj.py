...
def generate_pair_report(
    ...
    monthly_sum = df.groupby("Month")[["Generation", variable_name]].sum()
    monthly_sum = monthly_sum.round(3)
    monthly_sum.to_csv(
        os.path.join(folder, f"report_monthly_sum_{safe_name}.csv")
    )
    
    monthly_mean = df.groupby("Month")[["Generation", variable_name]].mean()
    monthly_mean = monthly_mean.round(3)
    monthly_mean.to_csv(
        os.path.join(folder, f"report_monthly_mean_{safe_name}.csv")
    )

    print(f"Generated reports for: Generation vs {variable_name}")

def model_prediction(df:pd.DataFrame, target: str, perfect_fit: list[float], model_name: str, rmse: float, r2: float)->None:
    plt.figure(figsize=(14, 6))

    plt.subplot(1, 2, 1)
    plt.scatter(df[target], df["Prediction"], alpha=0.5, color="blue", label="Predictions")
    plt.plot(perfect_fit, perfect_fit, "r--", lw=2, label="Perfect Fit")
    ...
    plt.tight_layout()
    plt.savefig(f"out/{model_name}.png", dpi=300)
    plt.show()