...
# -----------------------------------------------------------------------------
# Parcel selection helpers (for 1D column)
# -----------------------------------------------------------------------------
def _mixed_layer_parcel_from_Tq(
    ...
    p must be sorted high->low pressure (surface first).
    We average T and q over the lowest `depth_hpa` layer.
    """
    p0 = p[0]
    p_top = (p0.magnitude - depth_hpa) * units.hPa

    # layer mask: p >= p_top (i.e., within lowest depth)
    layer = p >= p_top

    if np.count_nonzero(layer) < 2:
        # too few levels -> fallback to surface
...
def _most_unstable_parcel_safe(
    ...
    """
    Try MetPy's most_unstable_parcel; if it fails (sparse levels), fallback to surface.
    """
    Td = mpcalc.dewpoint_from_specific_humidity(p, T, q)
    try:
        # MetPy expects p sorted surface->top (which we have)
        p0, T0, Td0 = mpcalc.most_unstable_parcel(p, T, Td, depth=depth_hpa * units.hPa)
        return p0, T0, Td0
    except Exception:
        return p[0], T[0], Td[0]
...
def _convective_params_column(
    ...
    # ---- LFC/EL (may be nan if not found) ----
