...
Mask logic:
  - Score = sum of satisfied conditions (0..4)
...
SAVE_PLOTS = True
PLOT_DIR = os.path.join("plots", "Composite_Mask_04.2")
...
# Require at least 3 of 4 signals
MIN_SCORE = 3
...
def plot_mask_frame(ds_mask: xr.Dataset, t_idx: int):
    ...
    plt.colorbar(cf, ax=ax, label="Composite Score (0–4)")
...
def main():
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            "MIN_SCORE": MIN_SCORE,
            "notes": (
                "Composite Cu mask using CAI + Convergence_mean + "
                "Convergence_10m + DD_LowMean_1000_850 (low values favorable)."
            ),
        },
    )
