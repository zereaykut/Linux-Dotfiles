...
def main() -> None:
    ...
    df_projection = data_prep(f"data/extracted-projection-data/{data_list[1]}", skiprows=52)
    df = pd.merge(df_generation, df_projection, on=["Date"], how="outer")

    # --- Plot ---
    plot_generation_regplot(df, "TR", "Deneme", "data")
    plot_dual_axis_ts(df, "TR", "Deneme", "data")
