...
class TransformerEncoderBlock(layers.Layer):
    """
    Not used in this specific architecture (we use CNN as encoder), 
    but often part of full Transformers.
    """
    pass 

class TransformerDecoderLayer(layers.Layer):
    """
    The core block of the caption generator.
    1. Masked Self-Attention: Looks at previous words in the caption.
    2. Cross-Attention: Looks at the Image Features (CNN output).
    ...
    def call(self, inputs, encoder_outputs, training=False, mask=None):
        # 1. Self Attention (Caption attending to itself)
        # mask is required to prevent looking at future words
        out1 = self.self_att(inputs, inputs, attention_mask=mask)
        out1 = self.layernorm1(inputs + out1)
        
        # 2. Cross Attention (Caption attending to Image Features)
        # encoder_outputs = Image Features
...
class ImageCaptioningModel(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape() as tape:
            ...
            # Loop over the 5 captions per image
            for i in range(self.num_captions_per_image):
                # Input to decoder: Sequence up to N-1
                inp = batch_seq[:, i, :-1]
                # Target: Sequence from 1 to N
                true = batch_seq[:, i, 1:]
                
                # Get mask (ignore padding 0s)
...
class ImageCaptioningSystem:
    def __init__(
        ...
        """
        Initializes the system with hyperparameters.
        """
    ...
    def _custom_standardization(self, input_string):
        """Preprocesses text: lowercase, strip punctuation."""
    ...
    def _load_image(self, img_path):
        ...
        return img / 255.0  # Normalize to [0, 1] for EfficientNet
    ...
    def prepare_data(self, image_paths, captions_list, validation_split=0.2):
        """
        Prepares dataset and text vectorization.
        image_paths: List of file paths.
        captions_list: List of List of strings (5 captions per image).
        """
        ...
        # Flatten captions for fitting vectorizer
        flat_captions = [cap for caps in captions_list for cap in caps]
        self.vectorization.adapt(tf.data.Dataset.from_tensor_slices(flat_captions).batch(1024))
    ...
    def build_model(self):
        """Constructs the Encoder (CNN) and Decoder (Transformer)."""
        logger.info("Building model architecture...")
        
        # 1. Image Encoder (EfficientNet)
        # We reshape output to (Batch, N*N, Features) for Attention
        ...
        img_features_input = layers.Input(shape=(None, self.embed_dim)) # Sequence of image features
    ...
    def generate_caption(self, image_path):
        ...
        # 3. Start Sequence
        # Note: Vectorizer must map "start" -> index. 
        # Usually it's in the vocab if dataset had "<start>".
        ...
        for i in range(self.seq_length - 1):
            ...
            # Get token with highest probability for the current step
...
if __name__ == "__main__":
    # 1. Setup Data (Dummy data for demonstration)
    # In a real scenario, you parse your folder to get these lists.
    # Note: Captions MUST include <start> and <end> tokens.
    
    # Create dummy images if not exist
    if not os.path.exists("test_img.jpg"):
        dummy = np.random.randint(0, 255, (299, 299, 3)).astype("uint8")
        import PIL.Image
        PIL.Image.fromarray(dummy).save("test_img.jpg")

    img_paths = ["test_img.jpg"] * 10
    captions = [
        ["<start> a dog running <end>", "<start> dog plays <end>", "<start> puppy <end>", "<start> animal <end>", "<start> dog <end>"]
    ] * 10
    ...
    # 5. Train
    captioner.train(epochs=2)
    
    # 6. Predict
    caption = captioner.generate_caption("test_img.jpg")
